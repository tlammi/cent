...
class LinuxDriver final : public cent::fs::Driver {
 public:
    LinuxDriver() {}
    std::string_view name() const noexcept override { return "linux"; }

    std::error_code mkdir(const stdfs::path& path, bool parents) override {
        std::error_code ec;
        if (parents) {
...
std::shared_ptr<Driver> make_linux_driver() {
    return std::make_shared<LinuxDriver>();
}
}  // namespace cent::fs