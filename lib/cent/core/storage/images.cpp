...
#include "cent/core/storage/images.hpp"
...
Images::Images(const nlohmann::json& json) {
    for (const auto& obj : json) {
        ...
        std::vector<oci::Reference> images{};
...
auto Images::at(const oci::Reference& image) -> Entry& {
...
auto Images::at(const oci::Reference& image) const -> const Entry& {
...
auto Images::operator[](oci::Reference image) -> Entry& {
    for (auto& e : m_entries) {
        for (auto& i : e.image_names) {
            if (i.str() == image.str()) return e;
        }
...
auto Images::operator[](DigestView digest) -> Entry& {
    ...
    m_entries.push_back(
        Entry{std::vector<oci::Reference>{}, std::move(digest)});
    return m_entries.back();
}
