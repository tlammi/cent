...
void serialize_uidmap(std::stringstream& ss, const std::vector<IdMap>& maps) {
...
class NamespaceDriver final : public Driver {
 public:
    std::unique_ptr<Sandbox> sandbox() override {
        return std::make_unique<NamespaceSandbox>();
    }

    std::string_view name() const noexcept override { return "namespace"; }

 private:
};

std::shared_ptr<Driver> make_namespace_sandbox_driver() {
    return std::make_shared<NamespaceDriver>();
}
}  // namespace cent::sandbox