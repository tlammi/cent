...
size_t BlobOut::write(std::span<const std::byte> buf) {
    assert(buf.size() <= std::numeric_limits<int>::max());
    auto res = sqlite3_blob_write(m_b.raw(), buf.data(), buf.size(), m_offset);
    check(res);
    m_offset += buf.size();
    return buf.size();
}
...
size_t BlobIn::size() { return sqlite3_blob_bytes(m_b.raw()); }

size_t BlobIn::read(std::span<std::byte> buf) {
    assert(buf.size() <= std::numeric_limits<int>::max());
    auto res = sqlite3_blob_read(m_b.raw(), buf.data(), buf.size(), m_offset);
    if (res == SQLITE_ERROR) {
        auto bytes = sqlite3_blob_bytes(m_b.raw());
        return read(buf.subspan(0, bytes));
    }
    check(res);
    return buf.size();
}
