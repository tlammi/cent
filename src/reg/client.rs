...
impl Client {
    ...
    pub fn manifest_list(&mut self, ref_: &ReferenceView) -> ManifestList {
        let body = self
            .fetch(&ref_.manifest_url(), mime::DOCKER_MANIFEST_LIST)
            .text()
            .unwrap();
    ...
    pub fn manifest(&mut self, ref_: &ReferenceView) -> Manifest {
        let body = self
            .fetch(&ref_.manifest_url(), mime::DOCKER_MANIFEST)
            .text()
            .unwrap();
    ...
    pub fn blob(&mut self, ref_: &ReferenceView, mime: MimeView) -> Vec<u8> {
        let body = self.fetch(&ref_.blob_url(), mime);
        body.bytes().unwrap().to_vec()
    }

    fn fetch(&mut self, url: &String, accept: MimeView) -> reqwest::blocking::Response {
